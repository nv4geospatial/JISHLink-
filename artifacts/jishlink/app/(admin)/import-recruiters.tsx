...
const FIELD_OPTIONS = [
  "recruiter_id", "full_name", "dob", "gender", "blood_group", "marital_status", "qualification",
  ...
  "workplace", "username", "password", "skip",
];

const AUTO_MAP: Record<string, string> = {
  recruiter_id: "recruiter_id", id: "recruiter_id", rec_id: "recruiter_id",
...
export default function ImportRecruitersScreen() {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const headerKeywords = ["recruiter_id", "full_name", "dob", "gender", "blood_group", "marital_status", "qualification", "contact_number", "email", "address", "emergency_contact", "nominee_name", "nominee_relation", "designation", "employment_type", "date_of_joining", "aadhar_number", "pan_number", "pf_number", "esi_number", "uan_number", "bank_name", "bank_branch", "account_number", "ifsc_code", "driving_license_number", "vehicle_details", "username", "password"];
  ...
  const handleImport = async () => {
    ...
    setLoading(true);
    try {
      const res = await apiFetch("/recruiters/import", {
        method: "POST",
      ...
      Toast.show({ type: "success", text1: `Imported ${res.success_count} recruiters` });
    } catch (e: unknown) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: c.background }]}>
      <NavHeader title="Import Recruiters" showBack onBack={() => router.replace("/(admin)/dashboard")} />
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: bottomPad + 32 }}>
        ...
        <View style={[styles.section, { backgroundColor: c.white }]}>
          ...
          <Text style={[styles.hint, { color: c.mutedForeground, fontFamily: "Inter_400Regular" }]}>
            Upload .xlsx or .csv file with recruiter details. Required: full_name, username, password.
          </Text>
        ...
        {showMapping && headers.length > 0 && (
          <View style={[styles.section, { backgroundColor: c.white }]}>
            ...
            <Text style={[styles.hint, { color: c.mutedForeground, fontFamily: "Inter_400Regular", marginBottom: 12 }]}>
              Click a pill to map each column. Recruiters will be created with role="recruiter".
            </Text>
            {headers.map((header) => (
            ...
            <TouchableOpacity onPress={handleImport} style={[styles.importBtn, { backgroundColor: c.gold }]} disabled={loading}>
              {loading ? <ActivityIndicator color={c.navy} /> : (
                <>
                  ...
                  <Text style={[styles.importBtnText, { color: c.navy, fontFamily: "Poppins_700Bold" }]}>Import {rows.length} Recruiters</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
