...
interface Employee {
  ...
  employment_status?: string | null; gender?: string | null;
...
export default function RecruiterExportScreen() {
  ...
  const handleExport = () => {
    ...
    const filename = `team_export_${new Date().toISOString().split("T")[0]}.csv`;
  ...
  return (
    <View style={[styles.container, { backgroundColor: c.background }]}>
      <NavHeader title="Export Team" showBack onBack={() => router.replace("/(recruiter)/dashboard")} />
      ...
      <FlatList
        ...
        ListEmptyComponent={
          isLoading ? <LoadingScreen /> : <EmptyState icon="users" title="No employees found" subtitle="Try a different search" />
        }
